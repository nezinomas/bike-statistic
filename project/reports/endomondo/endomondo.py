...
class MobileApi(object):
    ...
    def make_request(self, url, params={}, method='GET', data=None, **kwargs):
        ...
        if params.get('compression') == 'deflate':
            try:
                ...
                logging.warning(
                    'Could not decompress endomondo returned data, even thought deflate was requested. Error: %s' % e)
        '''
        try:
            data = r.json()
            if data.has_key('error'):
                logging.warning('Error loading data from Endomondo. Type: %s',
                                data['error'].get('type'))
    ...
    def post_account_info(self, account_info={}, **kwargs):
        ...
        r = self.make_request(URL_ACCOUNT_POST, params=kwargs,
    ...
    def post_workout(self, workout, properties={}):
        ...
        for chunk in chunks(workout.points, 100):
            ...
            r = self.make_request(URL_TRACK, params=properties,
                                  method='POST', data=data)

            lines = r.text.split("\n")
            if lines[0] != 'OK':
                raise EndomondoException(
                    'Could not post track. Error ``%s``. Data may be partially uploaded' % lines[0])
