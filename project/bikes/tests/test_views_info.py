...
def test_info_list_two_bikes(client_logged):
    info = BikeInfoFactory()
    BikeInfoFactory(
        component="XXX", description="YYY", bike=BikeFactory(short_name="z")
    )
...
def test_info_list_with_data_links(client_logged):
    ...
    url_update = reverse(
        "bikes:info_update", kwargs={"bike_slug": info.bike.slug, "pk": info.pk}
    )
    url_delete = reverse(
        "bikes:info_delete", kwargs={"bike_slug": info.bike.slug, "pk": info.pk}
    )
